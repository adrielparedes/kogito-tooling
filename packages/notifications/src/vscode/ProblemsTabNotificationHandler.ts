...
const diagnosticCollectionName = "kogito";

type NotificationSeverityConvertionType = {
  [K in NotificationSeverity]: vscode.DiagnosticSeverity;
};

const kogitoNotificationToVsCodeDiagnosticSeverityConvertionMap: NotificationSeverityConvertionType = {
...
export class ProblemsTabNotificationHandler implements NotificationsApi {
  private readonly diagnosticCollection = vscode.languages.createDiagnosticCollection(diagnosticCollectionName);
  ...
  private getSeverity(severity: NotificationSeverity): vscode.DiagnosticSeverity {
    const diagnostic = kogitoNotificationToVsCodeDiagnosticSeverityConvertionMap[severity];
